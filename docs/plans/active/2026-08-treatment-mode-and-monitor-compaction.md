...
> **ARCHITECTURE CORRECTION (29-08): Treatment is NOT a mode — it is a PART OF TEACHING.** The customer clarified the original build was a misconception. Reworked (build clean, 536 Core tests, runtime-verified end-to-end):
> - **Removed `OperatingMode.Treatment`** (enum value + TitleResourceKey case + MainScreen routing). It's gone from the mode picker/number-key shortcuts; a stale persisted "Treatment" mode falls back to Teaching gracefully. `mode_treatment` strings kept as the button/panel label (`AppStrings.TreatmentTitle`).
> - **A highlighted «Лечение» button** in the Teaching bottom bar (`MonitorControlPanel`, next to ЭОС) — always accent-green (stands out from plain neighbours), white outline while its panel is open. Mirrors the EOS button (`TreatmentClick` event + `SetTreatmentActive`).
> - **The panel is a right-docked overlay** over the shared Teaching monitor (`TreatmentPanelWindow`, a static Popup helper modeled on `EosWindow`) — no monitor of its own. `TreatmentScreen`→**`TreatmentPanel`** (panel-only UserControl; dropped `BuildRoot`/`MonitorView`/series-setup). Wired in MainScreen's Teaching case beside the EOS handler; closes + un-highlights + stops the pending-effect timer on ✕/re-tap, Heart3D-open, and monitor-dismiss.
> - **Seeds from the currently-displayed REAL rhythm** and re-seeds when the user selects a different one (`TreatmentRhythmMap.ClassifyByAcronyms` — inverse of the category→acronym map, +17 tests). **No abstract 12-state picker** (`BuildRhythmChangeCard`/`SetRhythm` UI removed). Interventions transition the real rhythm via the shared `RhythmViewModel.SelectRhythm` (a `_selfDrivingRhythm` guard stops the resulting change re-seeding the engine). Runtime-verified: seed Paced→«ЭКС», reselect→VT, sync-cardiovert VT→Sinus on the live monitor, ✕ closes.
> - ~~Note: the SR→representative resolver picks `ids[0]`, which can be a non-canonical sinus rhythm.~~ **Fixed 02-09:** new Core `Taxonomy.ResolveRepresentativePathologyId(acronym, pathologies)` picks the **purest** representative — a rhythm whose *primary* diagnosis (first acronym) IS the acronym beats one merely carrying it as a secondary tag (clean sinus over an SR-tagged AV-block entry), tie-broken by fewest findings → clinical-case number → id (deterministic). `TreatmentPanel.ShowRhythm` now uses it (+5 tests). Core engine/drug-catalog UNCHANGED.
>
> **Button behaviour (02-09, build clean, adversarial review clean, runtime-verified):** (a) «Лечение» is highlighted (accent) **only while its panel is open** — plain at rest, like ЭОС (was wrongly always-green). (b) The four overlay tools between the bottom-bar dividers — **pQRSt · ЭОС · Лечение · Подсказки** — are now **mutually exclusive** (only one on at a time): activating one switches the other three off (highlight + overlay/effect), via `MonitorControlPanel.DeactivateOverlayTools` (closes pQRSt/Tips on the view-model; closes the ЭОС/Лечение windows through host `RequestCloseEos`/`RequestCloseTreatment` callbacks whose close paths report `SetXActive(false)` back). Verified functionally (UIA: tap ЭОС→Treatment closes; tap Лечение→EOS closes) and visually. pQRSt's "no signal markup" notice-return correctly keeps it inactive on unmarked ECGs (so it doesn't deactivate others). A 2-dimension × verify review of the re-entrancy/ordering/state-sync returned zero findings.
>
> **Torsades: synthesized waveform + reachability (02-09, build clean, 548 Core tests, visually verified):** Closed the last real functional gap in the arrest states. Two findings drove it:
> - **Display** — the torsades state used to fall back to a **monomorphic PVT** (clinically wrong). Discovered `RhythmViewModel.ShowFlatline` builds its waveform **procedurally** (per-lead `float[]`, no pak/`.dat`), so the earlier "author a real `.dat`" pipeline block does **not** apply. Added a pure, unit-tested Core generator **`SyntheticEcg.Torsades`** (rotating-vector model: a fast ~250 bpm QRS-magnitude bump train projected onto each lead's hexaxial axis while the axis slowly rotates → an authentic twisting spindle with per-lead phase-offset nodes) and **`RhythmViewModel.ShowTorsades`** (mirrors the flatline synthesis). `TreatmentRhythmMap`: Torsades now lists **only `TDP`** (dropped the wrong PVT fallback) + `IsSynthesizedTorsades`; `TreatmentPanel.ShowRhythm` prefers a real TDP rhythm if ever authored, else synthesizes. Visual proof rendered from the real generator (II/aVF/V3 twist out of phase — scratchpad `tdpviz`).
> - **Reachability** — found Torsades was **unreachable**: the engine only ever *acted on* it (defib/magnesium), nothing transitioned *into* it, and the pak has no `TDP` rhythm to seed from — so both the display and the magnesium/defib torsades rules were dead paths. **Customer decision (02-09): amiodarone-induced TdP.** Added a QT-prolongation proarrhythmia to `TreatmentEngine`: amiodarone given for an organized tachyarrhythmia (VT-with-pulse / AFib / rate-controlled AFib) has a **5%** chance of inducing Torsades (drawn before the therapeutic effect; `&&` short-circuits so no other path's RNG changes). Makes the whole teaching loop live: **amio → TdP → twisting trace → magnesium rescue → sinus.** +8 tests (2 existing amiodarone tests updated for the extra draw).
>
> **Panel compaction — fit without scrolling (02-09, build clean, runtime-verified):** the panel used to be a single tall column that overflowed (only ~2½ cards visible before the fold) with a fixed ~200px mostly-empty log. Reworked so everything fits with no scroll: (a) **two balanced card columns** (left: IV drug, pacing, O₂/CPR toggles; right: defib, pill, vagal) + **full-width «Скорость»** row; (b) the host card now **sizes to its content** (capped at the available height, scroll only as an overflow safety) instead of stretching full-height — kills the empty gap and the giant empty log; (c) compacted paddings/spacing/fonts, panel widened 440→500 for the two columns, log trimmed to a 96px inner scroll; (d) slider rows converted to label|stretch-slider|value grids (fit any width, no clipping); (e) `Card` header is now a Grid so long titles WRAP instead of clipping («Кислород / ИВЛ», «СЛР — непрямой массаж сердца»); (f) pill card → 2 columns so names don't wrap mid-word; O₂/CPR use compact bare toggles. Runtime-verified end-to-end (screenshot): all controls visible, no scrollbar.
...
**Log save (29-08, build clean, runtime-verified):** a **«Сохранить»** button in the event-log card header exports the session log to a user-chosen text file. Uses the app's inline `FileSavePicker` idiom (copied from `StudentsScreen.OnExportClickAsync` — `App.MainWindow` HWND interop, `FileIO.WriteTextAsync`; no picker plumbing threaded into the screen). The report = a small localized header (mode · «Журнал событий», timestamp, final rhythm via `TxStatusFormat`) then the events in **chronological** order (`_vm.Log` is newest-first → reversed). Empty-log guard + success/failure feedback via the screen's existing `Toast`/`InfoAsync`. Reused the 5-lang `common_save` label; 3 new EN+RU keys (`tx_save_log_ok/_failed/_empty`) → worklist now 91 strings. **Decision:** chose the self-contained file-export (idiom A) over a persistent session-store+review-screen (idiom B, mirrors `ExamResultStore`) — the store schema (student identity, grading) is a separate customer call and a store without a review UI is half a feature. Verified end-to-end: button renders, picker opens, a correctly-formatted RU report was written to disk, and the «Журнал сохранён: …» toast showed. Scout map: `scenario-save-scouting` workflow.

**Remaining:** ~~author real asystole/torsades .dat~~ — **done via synthesis (02-09):** asystole = flat-line, torsades = `SyntheticEcg.Torsades` (twisting spindle) + reachable via amiodarone-induced TdP; a real authored `.dat` for either is now only an optional higher-fidelity upgrade, not a functional gap; optional persistent session-store + review UI (idiom B) if the customer wants graded/reviewable sessions; **tx_* strings not translated into ZH/ES/HI** (fall back to English — consistent with those dicts being broadly partial; deferred: machine-translating clinical terms is risky, needs a translator); further polish (mockup Отмена/Применить commit-staging, scenario save?); Android sync. Not committed.
...
3. **Asystole & Torsades** → author dedicated `.dat` rhythms (+ taxonomy acronyms). *(Superseded 02-09: both are now drawn procedurally — asystole flat-line, torsades `SyntheticEcg.Torsades` — since `RhythmViewModel` synthesizes waveforms in code with no pak dependency. A real `.dat` stays an optional fidelity upgrade.)*
4. **Torsades reachability** (02-09) → **amiodarone-induced TdP** (QT-prolongation proarrhythmia, 5%): torsades was otherwise unreachable (nothing transitioned into it, no TDP rhythm to seed). Makes the display + magnesium/defib torsades rules live.
