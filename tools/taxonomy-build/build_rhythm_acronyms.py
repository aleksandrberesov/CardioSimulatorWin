...
def classify(title, record_group, tax_groups):
    """Returns (acronym, reason) or (None, reason). Picks the finding whose taxonomy group matches the
    record group (earliest match wins), else the highest-priority finding overall."""
    seen = {}  # acronym -> (position, priority)
    for rx, acr, prio in COMPILED:
        m = rx.search(title)
        if m and acr not in seen:
            seen[acr] = (m.start(), prio)
    if not seen:
    ...
    if group_matches:
        ...
        return group_matches[0][2], "group-match"

    # No finding sits in the record's group — take the most significant finding.
    best = max(seen.items(), key=lambda kv: kv[1][1])
    return best[0], "priority-fallback"
...
def main():
    ...
    with open(manifest_path, "r", encoding="utf-8") as f:
        for raw in f:
            ...
            if not pid:
                continue
            acr, reason = classify(title, group, tax_groups)
            if acr:
                rows.append((pid, acr))
                tagged += 1
    ...
    print("# id\tacronym   — generated by build_rhythm_acronyms.py")
    for pid, acr in rows:
        print(f"{pid}\t{acr}")

    print(f"\nTagged {tagged}, untagged {untagged} ({fallback} via priority-fallback).", file=sys.stderr)
