    ...
    public bool WritePathology(PathologyFile file, IReadOnlyList<Lead>? leadOrder = null)
    {
        try
        {
            ...
            if (manifest is not null)
            {
                    ...
                    if (existing.TitleEn != file.TitleEn || existing.NameRu != file.NameRu || existing.Group != file.Group || existing.ClinicalCase != file.ClinicalCase || existing.Number != file.Number)
                    {
                        updatedEntries = manifest.Entries
                            .Select(e => e.Id == file.Id
                                ? e with { TitleEn = file.TitleEn, NameRu = file.NameRu, Group = file.Group, ClinicalCase = file.ClinicalCase, Number = file.Number }
                                : e)
                            .ToList();
                    }
                }
                ...
                {
                    updatedEntries = manifest.Entries
                        .Append(new PathologyEntry(file.Id, file.TitleEn, file.NameRu, file.Leads.Count, $"{file.Id}.dat", file.Group, file.ClinicalCase, file.Number))
                        .ToList();
                }
